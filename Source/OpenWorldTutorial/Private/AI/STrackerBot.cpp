...
void ASTrackerBot::SelfDestruct()
{
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		// Increase damage basesd on the power level
...
void ASTrackerBot::OnCheckNearbyBots()
{
	...
	if (MatInst)
	{ 
		// Convert to a float between 0 and 1 just like an 'Alpha' value of a texture. Now the material can bo set up without having to know to max power level
		...
		// otherwise the following happens when dealing when dividing intergers : 1 / 4 = 0('PowerLevel' int / 'MaxPowerLevel' int = 0 int)
...
void ASTrackerBot::NotifyActorBeginOverlap(AActor * OtherActor)
{
	...
	{
		AMyCharacter* PlayerPawn = Cast<AMyCharacter>(OtherActor);
