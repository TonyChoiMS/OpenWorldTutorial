...
// TODO:: GetCVar와 IsCVarRegistered 함수 정리?
IConsoleVariable* UConsoleUtils::GetCVar(FName Name)
{
	...
	if (!CVar)
	{
		FTCGameSettingsError::LogError(FString::Printf(TEXT("Tried to find console variable '%s' that doesn't exist"), *Name.ToString()));
	}
...
int32 UConsoleUtils::GetIntCVar(FName Name)
{
	...
	{
		FTCGameSettingsError::LogMissingCVar("Get Int CVar", Name);
		return 0;
	}
...
bool UConsoleUtils::GetBoolCVar(FName Name)
{
	if (!IsCVarRegistered(Name))
	{
		FTCGameSettingsError::LogMissingCVar("Get Bool CVar", Name);
		return false;
	}
...
float UConsoleUtils::GetFloatCVar(FName Name)
{
	if (!IsCVarRegistered(Name))
	{
		FTCGameSettingsError::LogMissingCVar("Get Float CVar", Name);
		return 0.f;
	}
...
FString UConsoleUtils::GetStringCVar(FName Name)
{
	if (!IsCVarRegistered(Name))
	{
		FTCGameSettingsError::LogMissingCVar("Get String CVar", Name);
		return FString();
	}
...
void UConsoleUtils::SetIntCVar(FName Name, int32 Value)
{
	if (!IsCVarRegistered(Name))
	{
		FTCGameSettingsError::LogMissingCVar("Set Int CVar", Name);
		return;
	}
...
void UConsoleUtils::SetBoolCVar(FName Name, bool Value)
{
	if (!IsCVarRegistered(Name))
	{
		FTCGameSettingsError::LogMissingCVar("Set Bool CVar", Name);
		return;
	}
...
void UConsoleUtils::SetFloatCVar(FName Name, float Value)
{
	if (!IsCVarRegistered(Name))
	{
		FTCGameSettingsError::LogMissingCVar("Set Float CVar", Name);
		return;
	}
...
void UConsoleUtils::SetStringCVar(FName Name, FString Value)
{
	if (!IsCVarRegistered(Name))
	{
		FTCGameSettingsError::LogMissingCVar("Set String CVar", Name);
		return;
	}
