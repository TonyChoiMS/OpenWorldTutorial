...
#include "TCGameSettingWidget.h"

UTCGameSettingWidget::UTCGameSettingWidget(const FObjectInitializer& ObjectInitializer)
...
void UTCGameSettingWidget::Apply()
{
	ApplyInternal();
}

void UTCGameSettingWidget::Save()
{
	...
	USettingsManager::Get()->SaveSetting(FTCSettingData(CVarName, FinalConfigValue, SettingTags), false);
	bHasUnsavedChange = false;
}

void UTCGameSettingWidget::Cancel()
{
...
void UTCGameSettingWidget::NativePreConstruct()
{
	...
	if (!UConsoleUtils::IsCVarRegistered(CVarName))
	{
		if (IsDesignTime())
		{
			...
			// CVar might not be registered because PIE hasn't run yey
			return;
		}
		else
		{
			// Log and keep going
			FTCGameSettingsError::LogMissingCVar(GetName(), CVarName);
		}
	}
	...
	// Register sink
	IConsoleManager::Get().RegisterConsoleVariableSink_Handle(FConsoleCommandDelegate::CreateUObject(this, &UTCGameSettingWidget::ConsoleVariableSink));
}

void UTCGameSettingWidget::NativeConstruct()
{
	Super::NativeConstruct();
}

void UTCGameSettingWidget::ConsoleVariableSink()
{
	if (!UConsoleUtils::IsCVarRegistered(CVarName))
	{
		// If CVar is not registered log an error and ignore
		UE_LOG(LogTCGameSettings, Error, TEXT("CVar %s is not registered"), *CVarName.ToString());
	}
...
void UTCGameSettingWidget::UpdateSelection_Implementation(const FString& Value)
{
}

void UTCGameSettingWidget::ApplySettingValue(FString Value, bool bSaveIfPossible)
{
...
void UTCGameSettingWidget::ApplyInternal()
{
	...
	if (!USettingsManager::Get()->HasConfigValue(CVarName))
	{
		...
		USettingsManager::Get()->SaveSetting(FTCSettingData(CVarName, UConsoleUtils::GetStringCVar(CVarName), SettingTags), false);
	}
	...
	USettingsManager::Get()->ApplySetting(FTCSettingData(CVarName, FinalCVarValue));
	bHasUnappliedChange = false;
}

FString UTCGameSettingWidget::ProcessIncomingValue(FString Value) const
{
	if (ValueMask)
	{
...
FString UTCGameSettingWidget::ProcessOutgoingValue(FString Value, bool bRecombineWithConfigValue) const
{
	if (ValueMask)
	{
...
void UTCGameSettingWidget::SetToSettingValue(bool bPreferConfigValue)
{
	CurrentValue = ProcessIncomingValue(USettingsManager::GetValue(CVarName, bPreferConfigValue));
	DispatchUpdateSelection(CurrentValue);
}

void UTCGameSettingWidget::DispatchUpdateSelection(const FString& Value)
{
