...
#include "Misc/TCGameSettingsConfig.h"
...
FString USettingsManager::GetValue(FName Key, bool bPreferConfigValue)
{
	...
	{
		FTCGameSettingsError::LogMissingCVar("Get Value", Key);
		return FString();
	}
...
void USettingsManager::RegisterBoolCVarSetting(FName Name, bool DefaultValue, const FString& Help)
{
	...
	const FString ConfigValue = Get()->GetConfigValue(Name);
	const bool InitValue = ConfigValue.IsEmpty() ? DefaultValue : FTCGameSettingsStringUtils::IsTruthy(ConfigValue); // Also accept int too
...
void USettingsManager::SaveSettingStatic(FTCSettingData SettingData)
{
	Get()->SaveSetting(SettingData, true);
}

void USettingsManager::ApplySettingStatic(FTCSettingData SettingData)
{
...
{
	const FString IniName = GetDefault<UTCGameSettingsConfig>()->SettingsIniName;
	// Load Ini
...
void USettingsManager::SaveSetting(FTCSettingData SettingData, bool bApplySetting)
{
	if (!SettingData.Key.IsNone() && !SettingData.Value.IsEmpty())
	{
		...
		UE_LOG(LogTCGameSettings, Log, TEXT("Saving setting %s with value: %s, previous: %s"), *SettingData.Key.ToString(), *SettingData.Value, *Previous);
...
void USettingsManager::ApplySetting(FTCSettingData SettingData)
{
	if (!UConsoleUtils::IsCVarRegistered(SettingData.Key))
	{
		FTCGameSettingsError::LogMissingCVar("Apply Setting", SettingData.Key);
		return;
	}

	const FString Previous = UConsoleUtils::GetStringCVar(SettingData.Key);

	UE_LOG(LogTCGameSettings, Log, TEXT("Applying setting %s with value: %s, prevous: %s"), *SettingData.Key.ToString(), *SettingData.Value, *Previous);
	UConsoleUtils::SetStringCVar(SettingData.Key, SettingData.Value);
}

FString USettingsManager::GetSectionName()
{
	return GetDefault<UTCGameSettingsConfig>()->SettingsSectionName;
}
...
void USettingsManager::ApplySettingsFromConfig()
{
	...
	UE_LOG(LogTCGameSettings, Log, TEXT("Applied %i settings from config"), SettingsLoaded);
}
...
void USettingsManager::AutoDetectSettings(int32 WorkScale, float CPUMultiplier, float GPUMultiplier)
{
	...
	SaveSetting(FTCSettingData("sg.ResolutionQuality", FString::FromInt(State.ResolutionQuality)), false);
	SaveSetting(FTCSettingData("sg.ViewDistanceQuality", FString::FromInt(State.ViewDistanceQuality)), false);
	SaveSetting(FTCSettingData("sg.AntiAliasingQuality", FString::FromInt(State.AntiAliasingQuality)), false);
	SaveSetting(FTCSettingData("sg.ShadowQuality", FString::FromInt(State.ShadowQuality)), false);
	SaveSetting(FTCSettingData("sg.PostProcessQuality", FString::FromInt(State.PostProcessQuality)), false);
	SaveSetting(FTCSettingData("sg.TextureQuality", FString::FromInt(State.TextureQuality)), false);
	SaveSetting(FTCSettingData("sg.EffectsQuality", FString::FromInt(State.EffectsQuality)), false);
	SaveSetting(FTCSettingData("sg.FoliageQuality", FString::FromInt(State.FoliageQuality)), false);
}
