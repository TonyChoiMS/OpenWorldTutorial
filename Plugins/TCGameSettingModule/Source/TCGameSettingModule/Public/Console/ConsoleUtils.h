 ...
 */
UCLASS()
class TCGAMESETTINGMODULE_API UConsoleUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	// Register a new interger CVar
	static void RegisterIntCVar(FName Name, int32 DefaultValue, const FString& Help);

	// Register a new interger CVar using a boolean default value (false = 0, true = 1)
	...
	// Set value of CVar as float
	UFUNCTION(BlueprintCallable, Category = "Console")
	static void SetFloatCVar(FName Name, float Value);

	// Set value of CVar as FString
	UFUNCTION(BlueprintCallable, Category = "Console")
