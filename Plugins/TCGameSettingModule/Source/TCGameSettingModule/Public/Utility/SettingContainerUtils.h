...
class UWidget;
class UUserWidget;
class UTCGameSettingWidget;

/**
 * Static helper functions for operating on multiple settings at the same time
 */
UCLASS()
class TCGAMESETTINGMODULE_API USettingContainerUtils : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	static TArray<UTCGameSettingWidget*> GetChildSettings(UUserWidget* UserWidget, UWidget* Parent = nullptr);
