...
public:
	...
	FTCSettingData(FName Key, FString Value) : Key(Key), Value(Value)
	{}

	FTCSettingData(FName Key, FString Value, FGameplayTagContainer Tags) : Key(Key), Value(Value), Tags(Tags)
	{}
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FSettingSavedSignature, FTCSettingData, SettingData);

/**
 * Handles saving, loading, and applying settings
 */
UCLASS()
class TCGAMESETTINGMODULE_API USettingsManager : public UObject
{
	GENERATED_BODY()
...
public:
	...
	static void SaveSettingStatic(FTCSettingData SettingData);

	// Applies a setting
	UFUNCTION(BlueprintCallable, Category = "Settings", meta = (DisplayName = "Apply Setting"))
	static void ApplySettingStatic(FTCSettingData SettingData);
	...
	void SaveSetting(FTCSettingData SettingData, bool bApplySetting);
	...
	void ApplySetting(FTCSettingData SettingData);
