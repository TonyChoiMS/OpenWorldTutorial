...
#include "Misc/TCGameSettingsInputProjectConfig.h"

static FAutoConsoleCommand DumpPlayersCommand(
	TEXT("TCGameSettings.Input.DumpPlayers"),
...
static FAutoConsoleCommand TestLAyoutMergeCommand(
	TEXT("TCGameSettings.Input.TestLayoutMerge"),
...
static TAutoConsoleVariable<int32> CVarDebugMode(
	TEXT("TCGameSettings.Input.Debug"),
	0,
...
UInputMappingManager::UInputMappingManager()
{
	...
	{
		FConfigUtils::MigrateConfigPropertiesFromSection(this, TEXT("/Script/TCGameSettingsInput.InputMappingManager"));
	}
}
...
{
	return GetDefault<UTCGameSettingsInputProjectConfig>()->GetInputPresets();
}
...
const UTCGameSettingsInputConfig* UInputMappingManager::GetInputConfigStatic()
{
...
{
	UE_LOG(LogTCGameSettingsInput, Display, TEXT("----- DumpPlayers -----"));
	...
	for (int i = 0; i < Instance->RegisteredPlayerControllers.Num(); i++)
	{
		...
		UE_LOG(LogTCGameSettingsInput, Display, TEXT("PlayerController %i: "), i);
		if (IsValid(PC))
		{
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("    Object name: %s"), *PC->GetName());
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("    Human readable name: %s"), *PC->GetHumanReadableName());
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("    Implements ITCGameSettingsPlayer: %i"), (int32)PC->Implements<UTCGameSettingsPlayer>());
			FPlayerInputMappings InputMappings = Instance->FindPlayerInputMappings(PC);
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("    Player ID (if applicable): %s"), InputMappings.PlayerId.IsEmpty() ? TEXT("EMPTY") : *InputMappings.PlayerId);
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("    Key Group: %s"), *InputMappings.PlayerKeyGroup.ToString());
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("    Base Preset Tag: %s"), *InputMappings.BasePresetTag.ToString());
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("    Custom Mappings: %i"), (int32)InputMappings.MappingOverrides.GetTotalNumInputDefinitions());
			DumpLayout(InputMappings.MappingOverrides);
			FInputMappingLayout MergedLayout = InputMappings.BuildMergedMappingLayout();
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("    Merged Mappings: %i"), (int32)MergedLayout.GetTotalNumInputDefinitions());
			DumpLayout(MergedLayout);
		}
		else
		{
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("INVALID"));
		}
	}
	UE_LOG(LogTCGameSettingsInput, Display, TEXT("----- End DumpPlayers -----"));
}

void UInputMappingManager::TestLayoutMerge()
{
	UE_LOG(LogTCGameSettingsInput, Display, TEXT("----- TestLayoutMerge -----"));
	UInputMappingManager* Instance = Get();
	...
	UE_LOG(LogTCGameSettingsInput, Display, TEXT("----- End TestLayoutMerge -----"));
}
...
void UInputMappingManager::AddPlayerActionOverride(APlayerController* Player, const FInputActionKeyMapping& NewMapping, int32 MappingGroup, bool bAnyKeyGroup)
{
	...
	UE_LOG(LogTCGameSettingsInput, Log, TEXT("InputMappingManager: Adding action override: %s"), *NewMapping.ActionName.ToString());
...
void UInputMappingManager::AddPlayerAxisOverride(APlayerController* Player, const FInputAxisKeyMapping& NewMapping, int32 MappingGroup, bool bAnyKeyGroup)
{
	...
	UE_LOG(LogTCGameSettingsInput, Log, TEXT("InputMappingManager: Adding axis override: %s, Scale: %f"), *NewMapping.AxisName.ToString(), NewMapping.Scale);
...
FInputActionKeyMapping UInputMappingManager::GetPlayerActionMapping(APlayerController* Player, FName ActionName, int32 MappingGroup, FGameplayTag KeyGroup, bool bUsePlayerKeyGroup) const
{
	...
	{
		InputOverride = FPlayerInputMappings(GetDefault<UTCGameSettingsInputProjectConfig>()->AsWeakInterfacePtrConst());
	}
...
void UInputMappingManager::SetPlayerInputPreset(APlayerController* Player, FInputMappingPreset Preset)
{
	...
	UE_LOG(LogTCGameSettingsInput, Log, TEXT("Setting input preset for '%s', tag: %s"), *Player->GetHumanReadableName(), *PresetTag);
...
const UTCGameSettingsInputConfig* UInputMappingManager::GetInputConfig() const
{
	return GetDefault<UTCGameSettingsInputProjectConfig>();
}
...
void UInputMappingManager::RegisterPlayerController(APlayerController* Player)
{
	...
	UE_LOG(LogTCGameSettingsInput, Log, TEXT("Registering input overrides for %s"), *Player->GetHumanReadableName());
...
FPlayerInputMappings UInputMappingManager::FindPlayerInputMappings(APlayerController* Player) const
{
	...
	bool bFound = ITCGameSettingsPlayer::GetInputMappings(Player, FoundMappings);

	if (bFound)
	{
		UE_LOG(LogTCGameSettingsInput, VeryVerbose, TEXT("Found input mappings for %s with via IAutoSettingsPlayer::GetInputMappings"), *Player->GetHumanReadableName());
	}

	// Player ID string to compare
	const FString PlayerIdString = ITCGameSettingsPlayer::GetUniquePlayerIdentifier(Player);

	if (!bFound)
	{
		UE_LOG(LogTCGameSettingsInput, VeryVerbose, TEXT("Checking internal mappings for %s with ID %s"), *Player->GetHumanReadableName(), *PlayerIdString);
		...
		for (FPlayerInputMappings PlayerInputMapping : PlayerInputOverrides)
		{
			...
			{
				UE_LOG(LogTCGameSettingsInput, VeryVerbose, TEXT("Found existing input mappings"));
	...
	if (bFound)
	{
		FoundMappings.SetConfig(GetDefault<UTCGameSettingsInputProjectConfig>()->AsWeakInterfacePtrConst());
		return FoundMappings;
	}

	// Make default
	return FPlayerInputMappings(PlayerIdString, ITCGameSettingsPlayer::GetDefaultInputMappingPreset(Player).PresetTag, GetDefault<UTCGameSettingsInputProjectConfig>()->AsWeakInterfacePtrConst());
}

void UInputMappingManager::SavePlayerInputMappings(APlayerController* Player, FPlayerInputMappings& NewMappings)
{
	UE_LOG(LogTCGameSettingsInput, Log, TEXT("Saving input overrides for %s"), *Player->GetHumanReadableName());
	...
	// Custom save
	ITCGameSettingsPlayer::SaveInputMappings(Player, NewMappings);
}
...
void UInputMappingManager::OnRegisteredPlayerControllerDestroyed(AActor* DestroyedActor)
{
	...
	UE_LOG(LogTCGameSettingsInput, Verbose, TEXT("Registered Player Controller %s for player %s destroyed"), *PlayerController->GetName(), *PlayerController->GetHumanReadableName());
