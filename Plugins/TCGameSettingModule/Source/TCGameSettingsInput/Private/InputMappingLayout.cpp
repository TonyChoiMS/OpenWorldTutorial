#include "..\Public\InputMappingLayout.h"
#include "Misc/ITCGameSettingsInputConfig.h"
#include "GameFramework/PlayerController.h"
#include "Misc/TCGameSettingsInputLogs.h"

void FInputMappingLayout::SetConfig(TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig)
{
	Config = InConfig;
...
void FInputMappingLayout::SetMappings(TArray<FInputActionKeyMapping> ActionMappings, TArray<FInputAxisKeyMapping> AxisMappings)
{
	...
	for (const FInputAxisKeyMapping& Axis : AxisMappings)
	{
		...
		for (FInputMappingGroup& Group : MappingGroups)
		{
			if (Group.GetAllAxis(Axis.AxisName, Axis.Scale, Config->GetKeyGroupOfKey(Axis.Key), bIsAxisKey).Num() == 0)
			{
				TargetGroup = &Group;
				break;
			}
...
FInputMappingLayout FInputMappingLayout::ReplaceAction(const FConfigActionKeyMapping& Action, int32 MappingGroupId, bool bAnyKeyGroup)
{
	...
	// Mutiple mappings can have invalid key (unbound) so don't unbind in that case
	if (Action.Key.IsValid())
	{
		// Don't check AllowMultibleBindingsPerKey here - it will be checked in GetMappingGroupsToUnbind
		UnboundMappings = UnbindChord(Action.Key, GetMappingGroupsToUnbind(MappingGroupId), Action.bShift, Action.bCtrl, Action.bAlt, Action.bCmd);
	}

	// Replace action and collect any more unbound ones
	UnboundMappings.GetMappingGroup(MappingGroupId) += GetMappingGroup(MappingGroupId).ReplaceAction(Action, bAnyKeyGroup);

	return UnboundMappings;
}

FInputMappingLayout FInputMappingLayout::ReplaceAxis(const FConfigAxisKeyMapping& Axis, int32 MappingGroupId, bool bAnyKeyGroup)
{
	// Use first mapping group if none specified
	if (MappingGroupId < 0)
		MappingGroupId = 0;
	...
	if (Axis.Key.IsValid())
	{
		...
		UnboundMappings = UnbindChord(Axis.Key, GetMappingGroupsToUnbind(MappingGroupId));
	}

	// Replace axis and collect any more unbound ones
	UnboundMappings.GetMappingGroup(MappingGroupId) += GetMappingGroup(MappingGroupId).ReplaceAxis(Axis, bAnyKeyGroup);
...
{
	UE_LOG(LogTCGameSettingsInput, Display, TEXT("        Mapping Groups:"));
	if (GetMappingGroupsConst().Num() == 0)
	{
		UE_LOG(LogTCGameSettingsInput, Display, TEXT("            NONE"));
	}
	for (int MappingGroupIndex = 0; MappingGroupIndex < GetMappingGroupsConst().Num(); MappingGroupIndex++)
	{
		...
		UE_LOG(LogTCGameSettingsInput, Display, TEXT("            %i:"), MappingGroupIndex);

		UE_LOG(LogTCGameSettingsInput, Display, TEXT("                Actions:"));
		...
		{
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("                    %i: %s"), ActionIndex, *MappingGroup.ActionMappings[ActionIndex].ToDebugString());
		}

		UE_LOG(LogTCGameSettingsInput, Display, TEXT("                Axes:"));
		...
		{
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("                    %i: %s"), AxisIndex, *MappingGroup.AxisMappings[AxisIndex].ToDebugString());
		}

		UE_LOG(LogTCGameSettingsInput, Display, TEXT("                Unbound Actions:"));
		...
		{
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("                    %i: %s"), ActionIndex, *MappingGroup.UnboundActionMappings[ActionIndex].ToDebugString());
		}

		UE_LOG(LogTCGameSettingsInput, Display, TEXT("                Unbound Axes:"));
		...
		{
			UE_LOG(LogTCGameSettingsInput, Display, TEXT("                    %i: %s"), AxisIndex, *MappingGroup.UnboundAxisMappings[AxisIndex].ToDebugString());
		}
	}
}
...
void FInputMappingLayout::Apply(APlayerController* Player)
{
	...
	// then add ones from GetActino() onto it (thus preserving any actions from the blacklist)
	...
	Player->PlayerInput->AxisMappings.Append(GetAxis(false));
