...
UBindCapturePrompt::UBindCapturePrompt(const FObjectInitializer& ObjectInitializer)
	...
	, CaptureMode(EBindingCaputreMode::OnReleased)
...
{
	SetKeyboardFocus();
	UE_LOG(LogTCGameSettingsInput, Verbose, TEXT("BindCapturePrompt : SetKeyboardFocus"));

	SetUserFocus(GetOwningPlayer());
	UE_LOG(LogTCGameSettingsInput, Verbose, TEXT("BindCapturePrompt: SetUserFocus to %s"), *GetOwningPlayer()->GetHumanReadableName());

	UE_LOG(LogTCGameSettingsInput, Log, TEXT("BindCapturePrompt: Listening for input"));
}
...
FReply UBindCapturePrompt::NativeOnKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	if (!ShouldIgnoreEvent(InKeyEvent))
	{
		...
		if (CaptureMode == EBindingCaputreMode::OnPressed)
		{
...
FReply UBindCapturePrompt::NativeOnKeyUp(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	...
	if (!ShouldIgnoreEvent(InKeyEvent))
	{
		if (CaptureMode != EBindingCaputreMode::OnReleased)
		{
			KeysDown.Remove(InKeyEvent.GetKey());
			return FReply::Handled();
		}
...
FReply UBindCapturePrompt::NativeOnMouseButtonDown(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	{
		UE_LOG(LogTCGameSettingsInput, Verbose, TEXT("BindCapturePrompt: NativeOnMouseButtonDown"));
...
FReply UBindCapturePrompt::NativeOnMouseButtonUp(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	if (!ShouldIgnoreEvent(InMouseEvent))
	{
		UE_LOG(LogTCGameSettingsInput, Verbose, TEXT("BindCapturePrompt: NativeOnMouseButtonUp"));
		if (CaptureMode != EBindingCaputreMode::OnReleased)
		{
			return FReply::Handled();
		}
...
FReply UBindCapturePrompt::NativeOnMouseMove(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	// If neither mouse axis are allowed, then don't event bother considering them
	...
	if (!ShouldIgnoreEvent(InMouseEvent))
	{
		...
		{
			UE_LOG(LogTCGameSettingsInput, Log, TEXT("BindCapturePrompt: Capture axis from mouse delta: %s"), *AccumulatedMouseDelta.ToString());
...
void UBindCapturePrompt::Capture(FKey PrimaryKey, float AxisScale)
{
	...
	{
		UE_LOG(LogTCGameSettingsInput, Log, TEXT("BindCapturePrompt: Escape key pressed: %s - Cancelling"), *PrimaryKey.ToString());
	...
	if (!UInputMappingManager::GetInputConfigStatic()->IsKeyAllowed(PrimaryKey))
	{
		...
		UE_LOG(LogTCGameSettingsInput, Log, TEXT("BindCapturePrompt: Ignored globally disallowed key: %s"), *PrimaryKey.ToString());
	...
	{
		// Primary key disallowed, abort
		UE_LOG(LogTCGameSettingsInput, Log, TEXT("BindCapturePrompt: Ignored key disallowed by IsKeyAllowed override: %s"), *PrimaryKey.ToString());
		RejectCapture(CapturedInput);
		return;
	}
	...
	{
		UE_LOG(LogTCGameSettingsInput, Log, TEXT("BindCapturePrompt: Rejecting key because not allowed by key grouop: %s"), *PrimaryKey.ToString());
		RejectCapture(CapturedInput);
		return;
	}

	UE_LOG(LogTCGameSettingsInput, Log, TEXT("BindCapturePrompt: Captured chord: %s, AxisScale: %f"), *Chord.GetInputText().ToString(), AxisScale);
