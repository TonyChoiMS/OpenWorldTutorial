...
void FInputMappingGroup::SetConfig(TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig)
{
	Config = InConfig;
}
...
TArray<FInputActionKeyMapping> FInputMappingGroup::GetAllActions(FName ActionName, FGameplayTag KeyGroup) const
{
	...
	{
		FTCGameSettingsError::LogError(FString::Printf(TEXT("Undefined Key Group with tag '%s'. Please add a Key Group with the tag in the Auto Settings page of the project settings."), *KeyGroup.ToString()));
	}
...
{
	TArray<FInputAxisKeyMapping> Axes = GetAllAxis(AxisName, Scale, KeyGroup);
	if (Axes.Num() == 0)
	{
...
FInputMappingGroup FInputMappingGroup::ReplaceAxis(FConfigAxisKeyMapping Axis, bool bAnyKeyGroup)
{
	...
	for (FInputAxisKeyMapping& ExistingAxis : GetAllAxis(Axis.AxisName, Axis.Scale, KeyGroup, bIsAxisKey))
	{
...
FInputMappingGroup FInputMappingGroup::FindUnboundMappings(const FInputMappingGroup& SourceMappingGroup) const
{
	...
	{
		if (GetAllAxis(Axis.AxisName, Axis.Scale, FGameplayTag(), false).Num() == 0)
		{
