...
UTexture* FKeyIconSet::GetIcon(FKey Key) const 
{
	...
	if (!FoundIcon)
	{
		UE_LOG(LogTCGameSettingsInput, Warning, TEXT("FKeyIconSet::GetIcon: Key icon defined but empty - Key: %s, tags: %s"), *Key.ToString(), *Tags.ToString());
	}

	return FoundIcon;
}

UTCGameSettingsInputConfig::UTCGameSettingsInputConfig()
	: bAutoInitializePlayerInputOverrides(true)
	, AllowModifierKeys(true)
	, AllowMultipleBindingsPerKey(false)
{
	CategoryName = "Plugings";

	ShiftModifierOverrideText = NSLOCTEXT("TCGameSettings", "ShiftKey", "Shift");
	CtrlModifierOverrideText = NSLOCTEXT("TCGameSettings", "CtrlKey", "Ctrl");
	AltModifierOverrideText = NSLOCTEXT("TCGameSettings", "AltKey", "Alt");
	CmdModifierOverrideText = NSLOCTEXT("TCGameSettings", "CmdKey", "Cmd");
...
TArray<FInputMappingPreset> UTCGameSettingsInputConfig::GetInputPresets() const
{
...
UTexture* UTCGameSettingsInputConfig::GetIconForKey(FKey InKey, FGameplayTagContainer Tags, float AxisScale) const
{
	UTexture* Result;
...
FText UTCGameSettingsInputConfig::GetKeyFriendlyName(FKey Key) const
{
...
bool UTCGameSettingsInputConfig::DoesKeyGroupContainKey(FGameplayTag KeyGroupTag, FKey Key) const
{
...
bool UTCGameSettingsInputConfig::SameKeyGroup(FKey KeyA, FKey KeyB) const
{
	return GetKeyGroupOfKey(KeyA) == GetKeyGroupOfKey(KeyB);
}

FKeyScale UTCGameSettingsInputConfig::GetAxisKey(FKey InButtonKey) const
{
...
FKey UTCGameSettingsInputConfig::GetAxisButton(FKey AxisKey, float AxisScale) const
{
...
bool UTCGameSettingsInputConfig::IsKeyAllowed(FKey Key) const
{
...
bool UTCGameSettingsInputConfig::ShouldBindingsBeUniqueBetweenMappingGroups(int32 MappingGroupA, int32 MappingGroupB) const
{
...
FInputMappingPreset UTCGameSettingsInputConfig::GetInputPresetByTag(FGameplayTag PresetTag) const
{
...
TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> UTCGameSettingsInputConfig::AsWeakInterfacePtrConst() const
{
	return TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface>(const_cast<ThisClass*>(this));
}

FGameplayTag UTCGameSettingsInputConfig::GetKeyGroupOfKey(FKey Key) const
{
	for (FKeyGroup KeyGroup : KeyGroups)
	{
...
bool UTCGameSettingsInputConfig::IsKeyGroupDefined(FGameplayTag KeyGroupTag) const
{
	return KeyGroups.ContainsByPredicate([KeyGroupTag](const FKeyGroup& KeyGroup) { return KeyGroup.KeyGroupTag == KeyGroupTag; });
}

bool UTCGameSettingsInputConfig::IsAxisKey(FKey Key) const
{
