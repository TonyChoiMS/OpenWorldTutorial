...
struct TCGAMESETTINGSINPUT_API FCapturedInput
{
	GENERATED_BODY()
...
UENUM()
enum class EBindingCaputreMode : uint8
{
 ...
 */
UCLASS(abstract)
class TCGAMESETTINGSINPUT_API UBindCapturePrompt : public UUserWidget
{
	GENERATED_BODY()
...
public:
	...
	// If true, only allow capturing keys in the passed in Key Group, otherwise the keys will be caputred anyway
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Bind Capture Prompt")
	bool bRestrictKeyGroup;

	// Should keys and mouse clicks be captured when they are pressed, or when they are released?
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Bind Capture Prompt")
	EBindingCaputreMode CaptureMode;
	...
	// Fire when an input chord is rejected
	UPROPERTY(BlueprintAssignable, Category = "Bind Capture Prompt")
	...
	UFUNCTION(BlueprintCallable, Category = "Bind Caputre Prompt")
	void Cancel();
