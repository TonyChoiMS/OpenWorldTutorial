...
struct TCGAMESETTINGSINPUT_API FInputMappingLayout
{
	GENERATED_BODY()

	FInputMappingLayout() {}

	FInputMappingLayout(TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig) 
		: Config(InConfig) 
	{}

	FInputMappingLayout(TArray<FInputMappingGroup> InMappingGroups, TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig) 
   ...
   // Returns a layout containing any mappings that were unbound
	...
	TArray<FInputAxisKeyMapping> GetAxis(bool bIncludeNullMappings = true) const
	{
		TArray<FInputAxisKeyMapping> AxisGroup;
		for (FInputMappingGroup Group: MappingGroups)
		{
	...
	int32 GetTotalNumInputDefinitions() const { return GetActions().Num() + GetAxis().Num(); }
