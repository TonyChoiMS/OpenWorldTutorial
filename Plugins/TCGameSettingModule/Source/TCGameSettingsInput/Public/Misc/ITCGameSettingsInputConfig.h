...
struct FMappingGroupLink
{
	...
	FMappingGroupLink() {}
	FMappingGroupLink(const TArray<int32>& MappingGroups) : MappingGroups(MappingGroups) {}
...
struct FAxisAssociation
{
	...
	FAxisAssociation() {}
	FAxisAssociation(FKey InAxisKey, TArray<FKeyScale> InButtonKeys) : AxisKey(InAxisKey), ButtonKeys(InButtonKeys) {}
...
class TCGAMESETTINGSINPUT_API UTCGameSettingsInputConfig : public UDeveloperSettings, public ITCGameSettingsInputConfigInterface
{
	GENERATED_BODY()

public:
	UTCGameSettingsInputConfig();
	...
	UPROPERTY(config, EditAnywhere, Category = Input, meta = (EidtCondition = "AllowModifierKeys"))
	...
	TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> AsWeakInterfacePtrConst() const;

	// ITCGameSettingsInputConfigInterface
	...
	virtual TArray<FName> GetPreservedAxes() const override { return PreservedAxis; }
	...
	// End ITCGameSettingsInputConfigInterface
...
protected:
	...
	UPROPERTY(config, meta = (DeprecatedProperty))
	TArray<FName> BlacklistedAxis_DEPRECATED;
};