...
// Holds saved inp0ut mappings for a specific player, as well as if they are custom or from a preset
// Used to save and load input mapping information
USTRUCT(BlueprintType)
struct TCGAMESETTINGSINPUT_API FPlayerInputMappings
{
	GENERATED_BODY()

	FPlayerInputMappings();

	FPlayerInputMappings(FString PlayerId, FGameplayTag InBasePresetTag, TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig);
	FPlayerInputMappings(bool bNullBasePreset, TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig);
	FPlayerInputMappings(TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig);
	...
	TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> Config;

	void SetConfig(TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig);
