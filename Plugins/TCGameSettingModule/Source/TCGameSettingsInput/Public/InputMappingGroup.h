...
class ITCGameSettingsInputConfigInterface;
...
USTRUCT()
struct TCGAMESETTINGSINPUT_API FInputMappingGroup
{
	GENERATED_BODY()

	FInputMappingGroup() {}

	FInputMappingGroup(TWeakInterfacePtr<const ITCGameSettingsInputConfigInterface> InConfig)
	{
		SetConfig(InConfig);
	}
	...
	// Return all action mappings that match the given paramters
	TArray<FInputActionKeyMapping> GetAllActions(FName ActionName, FGameplayTag KeyGroup) const;

	// Return all axis mappings that match the given paramters
